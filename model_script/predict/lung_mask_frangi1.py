...
Example:
    python -m model_script.predict.lung_mask \
        ...
        --binarize_output
        python -m model_script.predict.lung_mask \
        --images_dir /projectnb/ec500kb/projects/Fall_2025_Projects/vessel_seg/data/images \
        --pred_dir /projectnb/ec500kb/projects/Fall_2025_Projects/vessel_seg/data/frangi \
        --output_dir /projectnb/ec500kb/projects/Fall_2025_Projects/vessel_seg/data/frangi_lungmasked \
        --binarize_output

        python -m model_script.predict.lung_mask_frangi1 \
        --images_dir /projectnb/ec500kb/projects/Fall_2025_Projects/vessel_seg/data/images \
        --pred_dir /projectnb/ec500kb/projects/Fall_2025_Projects/vessel_seg/data/frangi \
        --output_dir /projectnb/ec500kb/projects/Fall_2025_Projects/vessel_seg/data/frangi_lungmasked \
        --binarize_output 
...
def strip_prediction_id(pred_path: Path) -> str:
    """
    Extract image ID from frangi filename:
    frangi_image_XXX_label.nii.gz -> image_XXX
    """
    name = pred_path.name
    ...
    # remove frangi_ prefix
    if name.startswith("frangi_"):
        name = name[len("frangi_"):]
...
def mask_prediction(
    ...
    *,
    ...
    if backend_type == "inferer":
        ...
        lung_mask_np = inferer.apply(image_ct)
    else:
        lungmask_module, lungmask_model = lungmask_backend[1]
        lung_mask_np = lungmask_module.apply(image_ct, model=lungmask_model)
...
def main():
    ...
    output_dir = args.output_dir or Path("data/frangi_lungmasked")
    ...
    for pred_path in tqdm(preds, desc="Applying lungmask"):
        ...
        out_name = f"pred_frangi_lungmasked_{strip_prediction_id(pred_path)}.nii.gz"
